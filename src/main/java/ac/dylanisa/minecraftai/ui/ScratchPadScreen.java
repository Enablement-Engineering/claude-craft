...
import ac.dylanisa.minecraftai.network.ServerboundScratchPadUpdatePacket;
...
public class ScratchPadScreen extends ModularUIScreen {
    ...
    public ScratchPadScreen() {
        super(ModularUI.of(createUI()), Component.translatable("screen.minecraftai.scratchpad"));
    }
    ...
    private UIElement createTitleBar() {
        ...
        var title = new Label();
        title.setText(Component.translatable("screen.minecraftai.scratchpad.title"));
    ...
    private UIElement createButtonBar() {
        ...
        saveButton.setText(Component.translatable("screen.minecraftai.scratchpad.save"));
        saveButton.setOnClick(e -> saveContent());
        saveButton.layout(layout -> layout.width(60).height(25));
        buttonBar.addChild(saveButton);

        var clearButton = new Button();
        clearButton.setText(Component.translatable("screen.minecraftai.scratchpad.clear"));
        ...
        closeButton.setText(Component.translatable("screen.minecraftai.scratchpad.close"));
    ...
    private void onScratchPadSync(String content) {
        MinecraftAI.LOGGER.info("ScratchPadScreen: onScratchPadSync called with {} chars, textArea={}, isDirty={}",
            content != null ? content.length() : 0, textArea != null, isDirty);
        if (textArea != null && !isDirty) {
            List<String> lines = Arrays.asList(content.split("\n", -1));
            MinecraftAI.LOGGER.info("ScratchPadScreen: Setting {} lines in textArea", lines.size());
    ...
    private void loadContent() {
        ...
        MinecraftAI.LOGGER.info("ScratchPadScreen: Requesting scratch pad content from server");
        ...
        MinecraftAI.LOGGER.info("ScratchPadScreen: Cached content = '{}' ({} chars)",
    ...
    private void saveContent() {
        ...
        MinecraftAI.LOGGER.info("TextArea getValue returned: {} lines", lines != null ? lines.length : "null");

        String content = lines != null ? String.join("\n", lines) : "";
        MinecraftAI.LOGGER.info("Saving scratch pad content ({} chars): {}", content.length(),

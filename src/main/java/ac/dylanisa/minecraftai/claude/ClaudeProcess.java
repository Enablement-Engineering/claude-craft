    ...
    public CompletableFuture<String> run(
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            try {
                ...
                MinecraftAI.LOGGER.info("Closed stdin, waiting for output...");
                ...
                try (BufferedReader reader = new BufferedReader(
                    ...
                    MinecraftAI.LOGGER.info("Reading from process stdout...");

                    // Check if process exited immediately
                    if (!process.isAlive()) {
                        MinecraftAI.LOGGER.error("Process exited immediately with code: {}", process.exitValue());
                    }

                    String line;
                    while ((line = reader.readLine()) != null) {
                        MinecraftAI.LOGGER.info("Claude line: {} (length={})",
                            line.length() > 100 ? line.substring(0, 100) + "..." : line, line.length());

                        ClaudeStreamEvent event = ClaudeStreamEvent.parse(line);
                        if (event == null) {
                            MinecraftAI.LOGGER.warn("Failed to parse Claude event");
                            continue;
                        }

                        MinecraftAI.LOGGER.info("Parsed event type={}, text={}",
                            event.type(), event.text().length() > 50 ? event.text().substring(0, 50) + "..." : event.text());

                        // Capture session ID from init event
                        if (event.isInit() && event.sessionId() != null) {
                            this.sessionId = event.sessionId();
                            MinecraftAI.LOGGER.info("Got session ID: {}", sessionId);
                        }

                        // Stream text chunks to the callback
                        if (event.isTextDelta() && onTextChunk != null) {
                            MinecraftAI.LOGGER.info("Sending text chunk: {}", event.text());
                        ...
                        if (event.isResult()) {
                            ...
                            MinecraftAI.LOGGER.info("Got result: {}", result);
                ...
                MinecraftAI.LOGGER.error("Claude process error", e);
                if (onError != null) {
                    onError.accept(e);
                }
    ...
    private static String findClaudeCommand() {
        ...
        for (String candidate : candidates) {
            if (new java.io.File(candidate).canExecute()) {
                MinecraftAI.LOGGER.info("Found Claude CLI at: {}", candidate);
                return candidate;
            }
        }

        MinecraftAI.LOGGER.warn("Claude CLI not found in common locations, falling back to PATH");
        return "claude";
    }
}
